    ...
    >
      ...
      <div className="p-6 border-b border-slate-200/50 dark:border-slate-700/50">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 bg-gradient-to-r from-blue-600 to bg-purple-600 rounded-xl flex items-center justify-center shadow-lg">
              ...
              {!collapsed && item.submenu && expandeditems.has(item.id) && (
                <div className="ml-8 mt-2 space-y-1">
                  {item.submenu.map((subitem) => {
                    return <button className="w-full text-left p-2 text-sm text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-800/50 rounded-lg transition-all cursor-pointer">{subitem.label}</button>;
                  })}
                </div>
              )}
            </div>
        ...
        <div className="p-4 border-t border-slate-200/50 dark:border-slate-700/50">
          <div className="flex items-center space-x-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50">
            <img
              src={SaifImg}
              alt="user"
              className="w-10 h-10 rounded-full ring-2 ring-blue-500 object-cover shadow-lg"
            />
